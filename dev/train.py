...
def parse_args():
    ...
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default=os.environ.get("SM_CHANNEL_TRAIN", "/opt/ml/aihub"),
    )
    parser.add_argument(
        "--val_data_dir",
        type=str,
    ...
    for epoch in range(max_epoch):
        ...
        with tqdm(total=val_num_batches) as pbar:
            for img, gt_score_map, gt_geo_map, roi_mask in val_loader:
                pbar.set_description("[Validation {}]".format(epoch + 1))
                ...
                pbar.update(1)
                val_dict = {
                    "Validation Cls loss": extra_info["cls_loss"],
                    "Validation Angle loss": extra_info["angle_loss"],
                    "Validation IoU loss": extra_info["iou_loss"],
                }
                pbar.set_postfix(val_dict)
